...
const generatePrompt = (content) => {
  const prompt = `
      Given the following text, describe the brand personality:
...
const getBrand = async () => {
  try {
    ...
    // we only have one item but this is an array
    const generatedChoices = completion.data.choices[0].text.trim();
